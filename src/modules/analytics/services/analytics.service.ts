...
export async function getRevenueAnalytics(query: AdminRevenueQuery) {
  ...
  const [
    ...
    prisma.order.aggregate({
      where:  { status: OrderStatus.SUCCESS, ...dateFilter.paidAt ? { paidAt: dateFilter.paidAt } : {} },
    ...
    prisma.order.count({ where: { status: OrderStatus.SUCCESS, ...dateFilter } }),
    prisma.order.count({ where: { status: OrderStatus.FAILED, ...dateFilter } }),
...
function buildDateFilter(from?: string, to?: string): Prisma.TestAttemptWhereInput {
  if (!from && !to) return {};
  return {
    startedAt: {
...
async function getAttemptsByExam(dateFilter: Prisma.TestAttemptWhereInput) {
  const rows = await prisma.testAttempt.findMany({
    where:  dateFilter,
